...
import { g } from "../../worker/util/index.ts";
import { changeTracker } from "../../worker/db/changeTracker.ts";
import { ValueChangeCalculator } from "../../worker/core/team/ValueChangeCalculator.ts";
import {
	AI_TID,
...
//
// BASKETBALL ONLY, and that is a measured decision rather than an oversight.
// The calculator has two designed kinks - a below-average player has his value
// divided by twenty, and a value above 1 is raised to a per-sport EXPONENT of 7
// in basketball and 3 in football and baseball. At 7 an ordinary trade lands in
// the tens or hundreds and the kinks sit far below anything that matters; at 3
// it lands well under one, which is exactly where they are, so six of these
// nine sweeps wobble by a few tenths there. None of that wobble reaches a
// decision: the accept line was measured at every rating from 44 to 64 in all
// three sports and came out identical. The portable claim is therefore about
// the ANSWER rather than about dv, and it lives in
// decisionProperties.shared.ts, which runs everywhere.
//
...
export const runValuationProperties = () => {
	describe("what the trade valuation must never do", () => {
		...
		test("a bigger salary on the same player is never an improvement", async () => {
			...
			assertMonotonic(
				dvs,
				"down",
				"a fatter contract made him more attractive",
			);
		});
