...
export const processAwards = async ({
	...
	// Reshape the players after they are loaded and before any formula sees
	// them. The sportsbook uses this to score a race on where the season is
	// heading rather than on partial totals (see core/season/getAwardRaceOdds).
	transformPlayers?: (
		players: Awaited<ReturnType<typeof getPlayers>>["players"],
		teamInfos: Awaited<ReturnType<typeof getPlayers>>["teamInfos"],
	) => void;
}) => {
	...
	if (transformPlayers) {
		transformPlayers(players, teamInfos);
	}
