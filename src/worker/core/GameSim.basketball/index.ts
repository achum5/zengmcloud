...
class GameSim extends GameSimBase {
	...
	updatePlayersOnCourt({
		...
		for (const t of teamNums) {
			...
			for (const pp of getSortedIndexes(ovrsOnCourt)) {
				...
				// Nobody comes off under 2 minutes in - unless a plan is in charge,
				// which is allowed to (planDriven below), so the shortcut stands
				// aside for it.
				if (!plannedNow && !(p.stat.courtTime > 2) && !onCourtIsIneligible) {
					continue;
				}
				...
				const bench = plannedNow
					? [...this.team[t].player].sort(
							(a, b) => ovrs.get(b.id)! - ovrs.get(a.id)!,
						)
					: this.team[t].player;
	 ...
	 */
	updateSynergy() {
		// The maths moved to synergy.ts unchanged, so the front office can ask
		// what a lineup is worth off exactly the code that will grade it on game
		// night. See the header there.
		for (const t of teamNums) {
	...
	recordStat(
		...
		s: Exclude<Stat, "benchTime" | "courtTime" | "energy" | "min">,
		amt: number = 1,
	) {
		if (p !== undefined) {
			if (s === "gp") {
