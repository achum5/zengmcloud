...
const LiveCourt = ({
	...
	const sheenId = `${floorUid}-sheen`;
	const vignetteId = `${floorUid}-vignette`;
		...
		() => (
			<>
				<defs>
					...
					{/* Broadcast lighting: a soft overhead sheen down the middle of the
					    floor (the reflection of the light banks) and a gentle vignette
					    darkening the edges, so the hardwood reads as LIT and glossy
					    instead of flat-printed. */}
					<radialGradient id={sheenId} cx="50%" cy="42%" r="62%">
				...
				{floorDetail()}
				{/* Overhead light sheen on the wood (under the court lines, so they
				    stay crisp). */}
				<rect
					x={0}
					y={0}
				...
				{/* Edge vignette (over everything on the floor), so the lit center
				    falls off into shadow at the sidelines - depth, like a real arena. */}
				<rect
					x={0}
					y={0}
					width={COURT_W}
