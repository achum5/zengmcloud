...
import { headerMeasuredShift } from "./visualViewportHeader.ts";
import {
	keyboardLikelyOpen,
	tickerMeasuredShift,
...
describe("headerMeasuredShift", () => {
	// The reported state: layout viewport pinned to the document top, the
	// visual viewport panning inside it, the header parked off the top of the
	// screen and the user watching it happen.
	test("the field report: header at -scrollY with the offset agreeing", () => {
		assert.strictEqual(
			headerMeasuredShift({
				measuredTop: -16,
				currentShift: 0,
	...
	test("a healthy page gets no shift", () => {
		assert.strictEqual(
			headerMeasuredShift({
				measuredTop: 0,
				currentShift: 0,
	...
	test("the ghost offset is not corroborated and moves nothing", () => {
		// The incident that killed the old correction: offset 406 on a page
		// scrolled to 0. The two numbers disagree, so nothing is trusted.
		assert.strictEqual(
			headerMeasuredShift({
				measuredTop: 0,
				currentShift: 0,
	...
	test("pinch-zoom pan is not corroborated either", () => {
		// Zoomed in and panned: offset and scrollY are independent numbers, and
		// a header off the top of a zoomed view is normal zoom, not a fault.
		assert.strictEqual(
			headerMeasuredShift({
				measuredTop: -300,
				currentShift: 0,
	...
	test("never acts inside the settle window", () => {
		// Mid-flick a healthy header reads -scrollY on the main thread. Acting
		// on that is how the old correction wrote translateY(229) onto a
		// healthy page.
		assert.strictEqual(
			headerMeasuredShift({
				measuredTop: -229,
	...
	test("a stale shift on a healed page clears at the next settle", () => {
		// Healthy sticky (would read 0 unshifted) still carrying an old 229.
		assert.strictEqual(
			headerMeasuredShift({
				measuredTop: 229,
				currentShift: 229,
	...
	test("it settles instead of oscillating", () => {
		// After the field-report shift is applied, the next pass computes the
		// same number, not a doubling.
		assert.strictEqual(
			headerMeasuredShift({
				measuredTop: 0,
				currentShift: 16,
	...
	test("a displacement no ride can produce is refused", () => {
		assert.strictEqual(
			headerMeasuredShift({
				measuredTop: -900,
				currentShift: 0,
